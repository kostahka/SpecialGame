...
std::size_t get_sound_format_size(uint16_t format_value)
{
    static const std::map<int, std::size_t> format = {
        { SDL_AUDIO_U8, 1 },     { SDL_AUDIO_S8, 1 },     { SDL_AUDIO_S16LSB, 2 },
        { SDL_AUDIO_S16MSB, 2 }, { SDL_AUDIO_S32LSB, 4 }, { SDL_AUDIO_S32MSB, 4 },
            ...
            while (current_stream_index != stream_size)
            {
                ...
                else
                {
                    SDL_MixAudioFormat(current_stream,
                                       ...
                                       sound->volume);
                    sound->current_index += static_cast<uint32_t>(stream_size);
                    current_stream_index += static_cast<uint32_t>(stream_size);
                }
            }
        }
    }
